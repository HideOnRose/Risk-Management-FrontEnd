<template>
  <el-main style="font-size: 12px">
    ...
    >
      ...
      <el-table-column prop="timeRemaining" label="剩余期限">
      </el-table-column>
      <!--<el-table-column prop="address" label="地址" width="300">
      </el-table-column>
      <el-table-column prop="zip" label="邮编" width="120"> </el-table-column> -->
      <el-table-column prop="label" label="还款率">
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope" style="text-align: left">
          <el-button
            @click.native.prevent="open(scope.row.sid, scope.row.timeRemaining)"
            type="text"
            size="small"
          >
            警告
...
export default {
  ...
  methods: {
    getData() {
      this.$http
        .get("http://127.0.0.1:8000/loan_approval/after_loan")
        .then((res) => {
          if (res.code == 404) {
            console.log("获取数据失败");
          } else {
    ...
    open(id, time) {
      this.$prompt("请输入邮件内容", "提示", {
        ...
        cancelButtonText: "取消",
        inputValue: id + ",您的贷款即将在" + time + "天后到期，请及时还款！",
      })
        .then(({ value }) => {
